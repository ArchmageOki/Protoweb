...
const CONSENTS_DIR =
  process.env.CONSENTS_DIR || path.resolve(process.cwd(), "consents");
try {
...
const consentStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) =>
    cb(null, `${req.user.id}-consentimiento_generico.pdf`),
});
...
import {
  ...
  pgDeleteEvent,
  pgCompleteEvent,
  pgGetWhatsappSession,
  pgUpsertWhatsappSession,
  pgInsertWhatsappMessage,
  pgListWhatsappMessages,
  pgGetUserSettings,
  pgUpsertUserSettings,
  ...
  pgGetClientCompletionToken,
  pgListMessageTemplates,
  pgGetMessageTemplate,
  pgUpsertMessageTemplate,
  pgDeleteMessageTemplate,
} from "./db/pg.js";
import {
  buildAuthUrl,
  exchangeCode,
...
r.post("/clients", async (req, res) => {
  ...
  if (typeof body.mobile !== "string" || !body.mobile.trim())
    return res.status(400).json({ error: "mobile_required" });
  ...
  try {
    ...
    let msg = "create_failed",
      field = null;
    if (e.code === "23505") {
      // Violación de restricción única
      if (e.detail && e.detail.includes("mobile")) {
        msg = "duplicate_mobile";
        field = "mobile";
      } else if (e.detail && e.detail.includes("dni")) {
        msg = "duplicate_dni";
        field = "dni";
      }
    }
...
r.get("/clients/:id/appointments/completed", async (req, res) => {
  ...
  if (isNaN(lim) || lim <= 0) lim = 200;
  try {
    const items = await pgListCompletedEventsForClient(
      req.user.id,
      req.params.id,
      lim
    );
...
r.post("/events", async (req, res) => {
  ...
  if (tAmt !== null && tAmt !== undefined && tAmt !== "") {
    tAmt = Number(tAmt);
    if (!isFinite(tAmt) || tAmt < 0)
      return res.status(400).json({ error: "invalid_total_amount" });
  } else tAmt = null;
  if (pAmt !== null && pAmt !== undefined && pAmt !== "") {
    pAmt = Number(pAmt);
    if (!isFinite(pAmt) || pAmt < 0)
      return res.status(400).json({ error: "invalid_paid_amount" });
  } else pAmt = null;
  if (tAmt != null && pAmt != null && pAmt > tAmt)
    return res.status(400).json({ error: "paid_gt_total" });
  ...
  if (client_id) {
    ...
    console.log(
      "[client-completion] Evaluación cliente",
      c.id,
      "missing=",
      missing
    );
  ...
  const row = await pgCreateEvent(nanoid(), req.user.id, {
    title: title.trim(),
    description,
  ...
  getAccount(req.user.id).then((acc) => {
    if (acc) {
      createRemoteEvent(req.user.id, row)
        .then((googleEv) => {
          pgAttachGoogleEvent(
            req.user.id,
            row.id,
            googleEv.id,
            googleEv.etag || null
          ).catch((e) => console.error("[google][attach] fallo", e.message));
        })
...
r.post("/clients/:id/completion-link", async (req, res) => {
  const client = await pgGetClient(req.user.id, req.params.id);
  if (!client) return res.status(404).json({ error: "not_found" });
  // Buscar token activo existente (no usado, no expirado) - consulta directa
  let tokenRow = null;
  try {
    const q = await pool.query(
      `select * from client_completion_tokens where user_id=$1 and client_id=$2 and used=false and (expires_at is null or expires_at>now()) order by created_at desc limit 1`,
      [req.user.id, client.id]
    );
    tokenRow = q.rows[0] || null;
  } catch (e) {
    console.error("[completion-link][select] error", e.message);
  }
  if (!tokenRow) {
    // invalidar tokens anteriores (marcar used=true) para garantizar uno activo
    try {
      await pool.query(
        `update client_completion_tokens set used=true, used_at=now() where user_id=$1 and client_id=$2 and used=false`,
        [req.user.id, client.id]
      );
    } catch (e) {
      console.error("[completion-link][invalidate-prev]", e.message);
    }
    const tokenId = nanoid();
    tokenRow = await pgCreateClientCompletionToken(
      tokenId,
      req.user.id,
      client.id
    );
  ...
  const url = `${base}/completar-datos.html?token=${tokenRow.id}`;
  res.json({
    ok: true,
    token: tokenRow.id,
    url,
...
r.put("/events/:id", async (req, res) => {
  ...
  const row = await pgUpdateEvent(req.user.id, req.params.id, {
    title: title.trim(),
    description,
    start_at: s.toISOString(),
    end_at: e.toISOString(),
  ...
  getAccount(req.user.id).then((acc) => {
    if (acc && row.google_event_id) {
      patchRemoteEvent(req.user.id, row.google_event_id, row)
        .then((googleEv) => {
          if (googleEv.etag)
            pgUpdateGoogleEtag(req.user.id, row.id, googleEv.etag).catch((e) =>
              console.error("[google][etag] fallo", e.message)
            );
        })
...
r.post("/events/:id/complete", async (req, res) => {
  try {
    ...
    if (ev.is_completed) {
      return res.json({ ok: true, item: ev, already: true });
    }
    ...
    if (updated.client_id) {
      try {
        await pgRecalcClientCompletedStats(req.user.id, updated.client_id);
      } catch (e) {
        console.error("[events][complete][recalc]", e.message);
      }
    }
...
r.delete("/events/:id", async (req, res) => {
  ...
  if (existing.google_event_id) {
    getAccount(req.user.id).then((acc) => {
      if (acc) {
        deleteRemoteEvent(req.user.id, existing.google_event_id).catch((err) =>
          console.error("[google][delete] fallo", err.message)
        );
      }
    });
  }
  ...
  if (existing.is_completed && existing.client_id) {
    try {
      await pgRecalcClientCompletedStats(req.user.id, existing.client_id);
    } catch (e) {
      console.error("[events][delete][recalc]", e.message);
    }
  }
  res.json({ ok: true });
});
...
r.get("/integrations/google/status", async (req, res) => {
  ...
  if (acc && typeof acc.calendar_id === "string" && !acc.calendar_id.trim()) {
    ...
    try {
      await pool.query(
        "update google_calendar_accounts set calendar_id=null where user_id=$1 and (calendar_id=$2 or calendar_id='')",
        [req.user.id, acc.calendar_id]
      );
    } catch (_e) {}
...
r.get("/integrations/google/authurl", (req, res) => {
  try {
    ...
    res.status(400).json({ error: "authurl_failed" });
  }
});
...
r.post("/integrations/google/callback", async (req, res) => {
  ...
  if (!validateAndConsumeState(req.user.id, state))
  ...
  try {
    ...
    await upsertAccount(req.user.id, {
      ...data,
...
r.get("/integrations/google/callback", async (req, res) => {
  ...
  try {
    ...
    res.status(400).send("oauth_failed");
  }
});
...
r.delete("/integrations/google/account", async (req, res) => {
  try {
    // Eliminar cuenta
    await pool.query("delete from google_calendar_accounts where user_id=$1", [
      req.user.id,
    ]);
    // Borrar eventos locales (requisito: eliminarlos de la DB, permanecen en Google)
    await pool.query("delete from calendar_events where user_id=$1", [
      req.user.id,
    ]);
...
const WHATSAPP_SERVICE_BASE =
  process.env.WHATSAPP_SERVICE_BASE || "http://localhost:4001";
let waProcess = null;
...
async function ensureWhatsappServiceSpawn() {
  ...
  if (
    ...
    process.env.WHATSAPP_AUTO_SPAWN !== "true"
  )
    return;
  ...
  try {
    ...
    const r = await fetch(WHATSAPP_SERVICE_BASE + "/health", {
      signal: ctrl.signal,
    });
    clearTimeout(t);
    if (r.ok) {
  ...
  try {
    ...
    waProcess = spawn(cmd, ["run", "dev"], {
      cwd: waDir,
      stdio: "inherit",
      env: process.env,
    });
    waProcess.on("exit", (code, signal) => {
      waProcess = null;
      console.log("[whatsapp][autospawn] proceso terminado", code, signal);
    });
    console.log("[whatsapp][autospawn] lanzado en", waDir);
  } catch (e) {
    console.error(
      "[whatsapp][autospawn] fallo al lanzar",
      e.code || "",
      e.message
    );
    // Backoff si falla
...
// Enviar automáticamente mensaje de completar datos del cliente
r.post("/whatsapp/send-completion-request", async (req, res) => {
  try {
    const { client_id, event_data } = req.body || {};
    ...
    const client = await pgGetClient(req.user.id, client_id);
    if (!client) {
    ...
    if (!client.mobile) {
      return res.status(400).json({ error: "client_no_phone" });
    }

    // Normalizar el teléfono (agregar +34 si es necesario)
    let phoneNumber = client.mobile.trim();
    if (phoneNumber && !phoneNumber.startsWith("+")) {
      // Si son 9 dígitos, agregar +34
      if (phoneNumber.replace(/\D/g, "").length === 9) {
        phoneNumber = "+34" + phoneNumber.replace(/\D/g, "");
      } else {
        phoneNumber = "+" + phoneNumber.replace(/\D/g, "");
      }
    }

    console.log("[whatsapp-completion] Teléfono normalizado:", phoneNumber);

    // Obtener la plantilla del usuario
    const template = await pgGetMessageTemplate(
      req.user.id,
      "client_data_request"
    );
    if (!template) {
    ...
    const nombre =
      ...
      "Cliente";
    const start = new Date(event_data.start);
    const end = new Date(event_data.end);
    const pad = (n) => String(n).padStart(2, "0");
    const fecha = `${pad(start.getDate())}/${pad(
      start.getMonth() + 1
    )}/${start.getFullYear()}`;
    const hora_inicio = `${pad(start.getHours())}:${pad(start.getMinutes())}`;
    const hora_fin = `${pad(end.getHours())}:${pad(end.getMinutes())}`;

    // Aplicar reemplazos en la plantilla
    let message = template.content;
    const replacements = {
      nombre,
      titulo: event_data.title || "Tu cita",
      fecha,
      hora_inicio,
      hora_fin,
    };

    // Si la plantilla contiene {{enlace_completar_datos}}, generar token
    if (message.includes("{{enlace_completar_datos}}")) {
      console.log(
        "[whatsapp-completion] Generando token de completar datos..."
      );

      // Invalidar tokens anteriores para este cliente
      try {
        await pool.query(
          `
          UPDATE client_completion_tokens 
        ...
        console.error(
          "[whatsapp-completion] Error invalidando tokens:",
          e.message
        );
      }
      ...
      const tokenId = nanoid();
      try {
        await pgCreateClientCompletionToken(tokenId, req.user.id, client.id);
        const base = process.env.PUBLIC_APP_ORIGIN || "http://localhost:5173";
        const completionUrl = `${base}/completar-datos.html?token=${tokenId}`;

        replacements.enlace_completar_datos = completionUrl;
        console.log("[whatsapp-completion] Token generado:", tokenId);
        console.log(
          "[whatsapp-completion] URL de completar datos:",
          completionUrl
        );
      } catch (e) {
        console.error(
          "[whatsapp-completion] Error generando token:",
          e.message
        );
        replacements.enlace_completar_datos = "";
      }
    } else {
      replacements.enlace_completar_datos = "";
    }
    ...
    console.log("[whatsapp-completion] Enviando mensaje a:", phoneNumber);
    console.log(
      "[whatsapp-completion] Mensaje:",
      message.substring(0, 100) + "..."
    );

    const requestBody = {
      phone: phoneNumber, // ← Usar teléfono normalizado
      message: message,
      clientId: client.id,
      clientName: nombre,
    ...
    console.log(
      "[whatsapp-completion] Datos enviados al microservicio:",
      requestBody
    );

    // Función para enviar mensaje con reintentos automáticos
    const sendWhatsAppWithRetries = async (maxRetries = 3, delayMs = 5000) => {
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        console.log(
          `[whatsapp-completion] Intento ${attempt}/${maxRetries} de envío...`
        );

        await ensureWhatsappServiceSpawn();
        const response = await fetch(WHATSAPP_SERVICE_BASE + "/whatsapp/send", {
          method: "POST",
          headers: {
            Authorization: "Bearer " + req.accessToken,
            "Content-Type": "application/json",
        ...
        if (response.ok) {
          console.log("[whatsapp-completion] ✅ Mensaje enviado exitosamente");
          return { success: true, result };
        }

        // Si es error de sesión no lista y no es el último intento, reintenta
        if (result.error === "Sesión no lista" && attempt < maxRetries) {
          console.log(
            `[whatsapp-completion] ⏳ Sesión no lista, reintentando en ${
              delayMs / 1000
        ...
        // Si es otro error o es el último intento, devolver error
        console.error(
          "[whatsapp-completion] ❌ Error enviando mensaje:",
          result
        );
    ...
    const sendResult = await sendWhatsAppWithRetries();

    if (sendResult.success) {
      res.json({
        ok: true,
        sent: true,
        message: "Mensaje de completar datos enviado",
      });
    } else {
      res.status(sendResult.status || 500).json(sendResult.result);
    }
  } catch (e) {
    console.error("[whatsapp-completion] Error:", e.message);
...
r.put("/whatsapp/session", async (req, res) => {
  ...
  if (status && !["inactive", "connecting", "ready", "error"].includes(status))
  ...
  const saved = await pgUpsertWhatsappSession(req.user.id, {
    phone_number,
    status,
    session_json,
  });
...
r.get("/whatsapp/messages", async (req, res) => {
  ...
  if (isNaN(size) || !allowedSizes.includes(size)) size = 50;
  let p = parseInt(page, 10);
  if (isNaN(p) || p < 1) p = 1;
  const offset = (p - 1) * size;
  try {
    const { items, total } = await pgListWhatsappMessages(req.user.id, {
      limit: size,
      offset,
    });
...
r.post("/whatsapp/outbox", async (req, res) => {
  ...
  try {
    const row = await pgOutboxInsert(nanoid(), req.user.id, {
      client_id,
      phone: phone.replace(/[^0-9+]/g, ""),
      client_name,
      instagram,
      message_text: message_text.slice(0, 5000),
...
r.get("/whatsapp/history", async (req, res) => {
  try {
    const { items } = await pgListWhatsappMessages(req.user.id, {
      limit: 100,
      offset: 0,
    });
...
r.post("/whatsapp/messages", async (req, res) => {
  ...
  try {
    const row = await pgInsertWhatsappMessage(nanoid(), req.user.id, {
      client_id,
      calendar_event_id,
      phone: phone.replace(/[^0-9+]/g, ""),
      client_name,
...
r.get("/settings", async (req, res) => {
  ...
  const consent_fixed_elements = Array.isArray(row.consent_fixed_elements)
    ? row.consent_fixed_elements
    : [];
  const consent_signature_rect =
    row.consent_signature_rect &&
    typeof row.consent_signature_rect === "object" &&
    !Array.isArray(row.consent_signature_rect)
      ? row.consent_signature_rect
      : {};
  res.json({
    ok: true,
    settings: {
      ...
      auto_title_enabled:
        typeof row.auto_title_enabled === "boolean"
          ? row.auto_title_enabled
          : true,
...
r.get("/templates", async (req, res) => {
  try {
    const rows = await pgListMessageTemplates(req.user.id);
    res.json({ ok: true, items: rows });
  } catch (e) {
...
r.get("/templates/:key", async (req, res) => {
  try {
    const row = await pgGetMessageTemplate(req.user.id, req.params.key);
    if (!row) return res.status(404).json({ error: "not_found" });
    res.json({ ok: true, item: row });
  } catch (e) {
...
r.put("/templates/:key", async (req, res) => {
  ...
  const { label = null, content = null } = req.body || {};
  ...
  const safeLabel =
    typeof label === "string" && label.trim()
      ...
      : key;
  try {
    const row = await pgUpsertMessageTemplate(
      req.user.id,
      key,
      safeLabel,
    ...
    console.error("[templates][upsert] error", e);
    res.status(500).json({ error: "save_failed" });
  }
});
r.delete("/templates/:key", async (req, res) => {
  try {
    await pgDeleteMessageTemplate(req.user.id, req.params.key);
    res.json({ ok: true });
  } catch (e) {
...
r.put("/settings", async (req, res) => {
  ...
  try {
    ...
    const extra_checks = body.hasOwnProperty("extra_checks")
      ? body.extra_checks || {}
      : existing?.extra_checks || {};
    const clientes = body.hasOwnProperty("clientes")
      ? body.clientes || {}
      : existing?.clientes || {};
    const auto_title_config = body.hasOwnProperty("auto_title_config")
      ? body.auto_title_config || {}
      : existing?.auto_title_config || {};
    const auto_title_enabled = body.hasOwnProperty("auto_title_enabled")
      ? !!body.auto_title_enabled
      : typeof existing?.auto_title_enabled === "boolean"
      ? existing.auto_title_enabled
      : true;
    const business_needs_consent = body.hasOwnProperty("business_needs_consent")
      ? !!body.business_needs_consent
      : existing?.business_needs_consent || false;
    const consent_pdf_info = body.hasOwnProperty("consent_pdf_info")
      ? body.consent_pdf_info || {}
      : existing?.consent_pdf_info || {};
    const consent_field_map = body.hasOwnProperty("consent_field_map")
      ? body.consent_field_map || {}
      : existing?.consent_field_map || {};
    let consent_fixed_elements_raw = body.hasOwnProperty(
      "consent_fixed_elements"
    )
      ? body.consent_fixed_elements ?? []
      : existing?.consent_fixed_elements || [];
    // Si llega como string intentar parsear
    if (typeof consent_fixed_elements_raw === "string") {
      try {
        const parsed = JSON.parse(consent_fixed_elements_raw);
        if (Array.isArray(parsed)) consent_fixed_elements_raw = parsed;
      } catch (_e) {
        consent_fixed_elements_raw = [];
      }
    }
    // Si llega como objeto {id:{...}} convertir a array
    if (
      !Array.isArray(consent_fixed_elements_raw) &&
      consent_fixed_elements_raw &&
      typeof consent_fixed_elements_raw === "object"
    ) {
    ...
    if (Array.isArray(consent_fixed_elements_raw)) {
      consent_fixed_elements_raw = consent_fixed_elements_raw
        .map((el) => {
          if (typeof el === "string") {
            try {
    ...
    const consent_signature = body.hasOwnProperty("consent_signature")
      ? body.consent_signature || null
      : existing?.consent_signature || null;
    const consent_signature_rect_raw = body.hasOwnProperty(
      "consent_signature_rect"
    )
      ? body.consent_signature_rect || {}
      : existing?.consent_signature_rect || {};
    const consent_fixed_elements = Array.isArray(consent_fixed_elements_raw)
      ? consent_fixed_elements_raw
          ...
          .map((el) => ({
            ...
            fontSize:
              typeof el.fontSize === "number" && isFinite(el.fontSize)
                ? el.fontSize
                : 12,
            page: 1,
          }))
    ...
    let consent_signature_rect =
      consent_signature_rect_raw &&
      typeof consent_signature_rect_raw === "object" &&
      !Array.isArray(consent_signature_rect_raw)
        ? { ...consent_signature_rect_raw }
        : {};
    if (consent_signature_rect.x != null) {
      ["x", "y", "w", "h", "ratio"].forEach((k) => {
        if (
          typeof consent_signature_rect[k] !== "number" ||
          !isFinite(consent_signature_rect[k])
        )
          delete consent_signature_rect[k];
      });
      consent_signature_rect.page = 1;
    }
    const saved = await pgUpsertUserSettings(req.user.id, {
      extra_checks,
      clientes,
      auto_title_config,
      auto_title_enabled,
    ...
    res.json({
      ...
      settings: {
        ...
        consent_fixed_elements: Array.isArray(saved.consent_fixed_elements)
          ? saved.consent_fixed_elements
          : [],
        consent_signature: saved.consent_signature || null,
        consent_signature_rect:
          saved.consent_signature_rect &&
          typeof saved.consent_signature_rect === "object" &&
          !Array.isArray(saved.consent_signature_rect)
            ? saved.consent_signature_rect
            : {},
      },
    });
...
r.post(
  "/settings/consent-pdf",
  uploadConsent.single("file"),
  async (req, res) => {
    try {
      if (!req.file) return res.status(400).json({ error: "missing_file" });
      const stat = fs.statSync(req.file.path);
      const existing = await pgGetUserSettings(req.user.id);
      const consent_pdf_info = {
...
r.get("/settings/consent-pdf", async (req, res) => {
  try {
    ...
    res.status(400).json({ error: "fetch_failed" });
  }
});
