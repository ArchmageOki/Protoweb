...
import {
  ...
  pgMarkClientCompletionTokenUsed,
} from "./db/pg.js";
...
function tokenExpired(t) {
  ...
  if (t.expires_at) {
    return Date.now() > Date.parse(t.expires_at);
  }
  // fallback por compatibilidad si no existe expires_at
...
app.get("/public/client-completion/:token", async (req, res) => {
  try {
    console.log("[DEBUG] Buscando token:", req.params.token);
    const t = await pgGetClientCompletionToken(req.params.token);
    console.log("[DEBUG] Token encontrado:", t ? "SÍ" : "NO");

    if (!t || t.used || tokenExpired(t)) {
      console.log(
        "[DEBUG] Token inválido - usado:",
        t?.used,
        "expirado:",
        t ? tokenExpired(t) : "N/A"
      );
      return res.status(404).json({ error: "invalid_token" });
    }

    console.log(
      "[DEBUG] Buscando cliente - user_id:",
      t.user_id,
      "client_id:",
      t.client_id
    );
    const client = await pgGetClient(t.user_id, t.client_id);
    console.log("[DEBUG] Cliente encontrado:", client ? "SÍ" : "NO");
    ...
    const response = {
      ...
      expires_at:
        t.expires_at ||
        new Date(
          new Date(t.created_at).getTime() + 7 * 24 * 60 * 60 * 1000
        ).toISOString(),
    };
    ...
    console.error("[ERROR] Error en endpoint client-completion:", e.message);
...
app.post("/public/client-completion/:token", async (req, res) => {
  try {
    ...
    const updated = await pgUpdateClient(t.user_id, t.client_id, {
...
// Finaliza consentimiento: guarda datos del formulario, marca whatsapp_consent y consume token
app.post("/public/client-completion/:token/consent", async (req, res) => {
  try {
    ...
    const { whatsapp_consent = false, client_data = null } = req.body || {};

    const client = await pgGetClient(t.user_id, t.client_id);
    if (!client) return res.status(404).json({ error: "client_not_found" });

    // Si se proporcionan datos del cliente, actualizarlos
    let updateData = { whatsapp_consent: !!whatsapp_consent };
    ...
    if (client_data) {
      ...
      // Agregar datos del cliente a la actualización
      updateData = {
        ...updateData,
        first_name: first_name.trim(),
        last_name: last_name.trim(),
    ...
    await pgUpdateClient(t.user_id, t.client_id, updateData);
...
app.listen(PORT, () => {
  ...
  (async () => {
    try {
      await pool.query(
        "alter table google_calendar_accounts add column if not exists needs_reauth boolean not null default false"
      );
      const { rows } = await pool.query(
        "select user_id, expiry, needs_reauth from google_calendar_accounts"
      );
      for (const r of rows) {
        ...
        if (!r.expiry || Date.now() > Date.parse(r.expiry)) {
          ...
          try {
            await pool.query("select 1"); /* noop */
          } catch (_e) {}
        }
      }
    } catch (e) {
      console.error("[google][startup-refresh] error", e.message);
    }
  })();
  setInterval(async () => {
    try {
      const { rows } = await pool.query(
        "select user_id, needs_reauth from google_calendar_accounts"
      );
      for (const r of rows) {
        if (r.needs_reauth) continue;
        applySync(r.user_id).catch((e) =>
          console.error("[google][sync] fallo user", r.user_id, e.message)
        );
      }
    } catch (e) {
      console.error("[google][sync] scheduler error", e.message);
    }
  }, interval);
});
